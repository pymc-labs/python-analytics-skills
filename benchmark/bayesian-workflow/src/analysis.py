...
CRITERIA = [
    "iterative_approach", "prior_predictive", "model_criticism",
    "diagnostics", "reporting", "efficiency", "total",
]
...
def load_scores(scores_dir: Path | None = None) -> pl.DataFrame:
    ...
    for f in sorted(scores_dir.glob("*.json")):
        ...
        records.append({
            ...
            "rep": data["rep"],
            "iterative_approach": data.get("iterative_approach", 0),
            "prior_predictive": data.get("prior_predictive", 0),
            "model_criticism": data.get("model_criticism", 0),
            "diagnostics": data.get("diagnostics", 0),
            "reporting": data.get("reporting", 0),
    ...
    if not records:
        return pl.DataFrame(schema={
            ...
            "condition": pl.Utf8,
            "rep": pl.Int64,
            "iterative_approach": pl.Int64,
            "prior_predictive": pl.Int64,
            "model_criticism": pl.Int64,
            "diagnostics": pl.Int64,
            "reporting": pl.Int64,
            "efficiency": pl.Int64,
            "total": pl.Int64,
            "passed": pl.Boolean,
...
def summary_table(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .agg([
            pl.col("iterative_approach").mean().alias("iterative_mean"),
            pl.col("prior_predictive").mean().alias("prior_pred_mean"),
            pl.col("model_criticism").mean().alias("criticism_mean"),
            pl.col("diagnostics").mean().alias("diagnostics_mean"),
            pl.col("reporting").mean().alias("reporting_mean"),
...
def generate_report(
    ...
    lines = [
        "# Bayesian Workflow Skill Benchmark — Analysis Report",
        "",
        "## Summary",
        "",
    ...
    lines.append("| Task | Condition | N | Iterative | Prior Pred | Criticism | Diagnostics | Reporting | Efficiency | Total |")
    lines.append("|------|-----------|---|-----------|------------|-----------|-------------|-----------|------------|-------|")

    for row in summary.iter_rows(named=True):
        lines.append(
            f"| {row['task_id']} | {row['condition']} | {row['n']} | "
            f"{row['iterative_mean']:.1f} | {row['prior_pred_mean']:.1f} | "
            f"{row['criticism_mean']:.1f} | {row['diagnostics_mean']:.1f} | "
            f"{row['reporting_mean']:.1f} | {row['efficiency_mean']:.1f} | "
            f"{row['total_mean']:.1f} |"
        )
    ...
    lines.extend([
        ...
        "## Pass/Fail Summary",
        "",
        "A run passes if: sampling completed (>100 draws), diagnostics acceptable",
        "(diagnostics >= 2), and posterior estimates are finite and non-degenerate.",
        "",
    ...
    lines.extend([
        ...
        "## Retry Summary",
        "",
        "Number of error-fix cycles (model file rewrites) per run. Lower = better.",
        "",
