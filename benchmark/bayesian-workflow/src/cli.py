"""CLI entry point for the Bayesian Workflow skill benchmark.

Subcommands:
  run        — Run benchmark tasks
  score      — Score completed runs
  analyze    — Generate analysis report
  validate   — Gate: run W1 in both conditions, verify everything works
...
def cmd_score(args):
    ...
    for r in results:
        print(
            ...
            f"iter={r.iterative_approach} prior={r.prior_predictive} "
            f"crit={r.model_criticism} diag={r.diagnostics} "
            f"report={r.reporting} eff={r.efficiency} "
            f"total={r.total}/30"
        )
...
def cmd_validate(args):
    """Validation gate: run W1 in both conditions, verify everything works."""
    print("=" * 60)
    print("VALIDATION RUN — W1 radon, both conditions")
    print("=" * 60)
    ...
    for condition in ["no_skill", "with_skill"]:
        log(f"\n--- Running W1 {condition} ---")
        start = time.time()
        result = run_single("W1_radon", condition, rep=0, force=args.force)
    ...
    no_skill_dir = get_run_dir("W1_radon", "no_skill", 0)
    with_skill_dir = get_run_dir("W1_radon", "with_skill", 0)

    # Check 1: Both runs completed
    ...
    for cond, run_dir in [("no_skill", no_skill_dir), ("with_skill", with_skill_dir)]:
        try:
            score = score_run(run_dir, "W1_radon", cond, 0)
...
def main():
    parser = argparse.ArgumentParser(
        description="Bayesian Workflow Skill Benchmark",
    ...
    p_run.add_argument("--task", default="W1_radon", help="Task ID to run")
    ...
    p_score.add_argument("--task", default="W1_radon")
    ...
    # validate
    p_val = sub.add_parser("validate", help="Validation gate (W1, both conditions)")
