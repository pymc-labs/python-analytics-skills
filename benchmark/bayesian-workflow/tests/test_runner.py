...
def test_load_tasks():
    ...
    assert "W1_radon" in config["tasks"]
    assert "W2_dose_response" in config["tasks"]


def test_task_has_required_fields():
    """Each task has prompt, reference_workflow, data_files."""
    config = load_tasks()
    for tid, task in config["tasks"].items():
        assert "prompt" in task, f"{tid} missing prompt"
        assert "reference_workflow" in task, f"{tid} missing reference_workflow"
...
def test_build_command_no_skill():
    """no_skill command includes pymc-modeling skill only."""
    cmd = _build_command("no_skill")
    assert "--append-system-prompt" in cmd
    idx = cmd.index("--append-system-prompt")
    skill_content = cmd[idx + 1]
    # Should contain pymc-modeling skill but NOT bayesian-workflow
    assert len(skill_content) > 1000
    # Verify it's the pymc skill (check for a pymc-specific marker)
    assert "---" not in skill_content or "bayesian-workflow" not in skill_content.split("---")[-1]


def test_build_command_with_skill():
    """with_skill command includes both pymc-modeling and bayesian-workflow skills."""
    cmd = _build_command("with_skill")
    assert "--append-system-prompt" in cmd
    idx = cmd.index("--append-system-prompt")
    skill_content = cmd[idx + 1]
    # Should be longer than no_skill (contains both skills)
    no_skill_cmd = _build_command("no_skill")
    no_skill_idx = no_skill_cmd.index("--append-system-prompt")
    no_skill_content = no_skill_cmd[no_skill_idx + 1]
    assert len(skill_content) > len(no_skill_content)


def test_build_command_skill_paths_exist():
    """Verify both skill files exist."""
    assert PYMC_SKILL_PATH.exists(), f"PyMC skill not found: {PYMC_SKILL_PATH}"
    assert WORKFLOW_SKILL_PATH.exists(), f"Workflow skill not found: {WORKFLOW_SKILL_PATH}"


def test_get_run_dir():
    """Run dir follows naming convention."""
    d = get_run_dir("W1_radon", "no_skill", 2)
    assert d.name == "W1_radon_no_skill_rep2"
...
def test_verify_token_difference_ok():
    ...
    ws = {"num_turns": 6, "cache_creation_tokens": 12000}
...
def test_verify_token_difference_no_turns():
    ...
    ws = {"num_turns": 5, "cache_creation_tokens": 12000}
    failures = verify_token_difference(ns, ws)
    assert len(failures) == 1
...
def test_is_cached_no_dir(tmp_path):
    ...
    with patch("src.runner.RUNS_DIR", tmp_path):
        assert not is_cached("W1_radon", "no_skill", 0)


def test_is_cached_with_metadata(tmp_path):
    """Run dir with metadata.json is cached."""
    run_dir = tmp_path / "W1_radon_no_skill_rep0"
    run_dir.mkdir()
    (run_dir / "metadata.json").write_text("{}")
    with patch("src.runner.RUNS_DIR", tmp_path):
        assert is_cached("W1_radon", "no_skill", 0)
